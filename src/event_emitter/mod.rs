...
pub trait EventEmitter: Send + Sync {
    ...
    async fn on_account_presence(&mut self, _: &Room, _: &NonRoomEvent) {}
    /// Fires when `AsyncClient` receives a `NonRoomEvent::RoomName` event.
    async fn on_account_ignored_users(&mut self, _: &Room, _: &NonRoomEvent) {}
    ...
    /// Fires when `AsyncClient` receives a `NonRoomEvent::RoomAliases` event.
    async fn on_presence_event(&mut self, _: &Room, _: &PresenceEvent) {}
}
